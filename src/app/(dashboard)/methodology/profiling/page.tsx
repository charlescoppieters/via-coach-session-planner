...
export default function MyPositionalProfilingPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsLoading(false)
  }, [club?.id, coach?.id, selectedTeamId])
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToPositionalProfiles(club.id, selectedTeamId, () => {
      fetchData()
    })

    return unsubscribe
  }, [club?.id, selectedTeamId, fetchData])
  ...
  const handleRevert = async () => {
    ...
    setIsReverting(true)
    setError('')
    const { error } = await revertTeamPositionalProfiles(selectedTeamId, club.id)
      ...
      {showRevertConfirm && (
        ...
        >
          ...
          >
            ...
            >
              Revert to Club Positional Profiling
            </h2>
            <p
              style={{
                fontSize: theme.typography.fontSize.base,
