...
interface PlayerBalanceTabProps {
  trainingBalance: PlayerTrainingBalance[] | null
  isLoading: boolean
}
...
export const PlayerBalanceTab: React.FC<PlayerBalanceTabProps> = ({
  trainingBalance,
  isLoading,
}) => {
  if (isLoading) {
  ...
  if (!trainingBalance || trainingBalance.length === 0) {
    return (
      <div
        style={{
          textAlign: 'center',
      ...
      >
        ...
        <p style={{ fontSize: theme.typography.fontSize.sm }}>
          Complete sessions with tagged blocks to see training balance.
        </p>
      </div>
    )
  }
  ...
  const totalOpportunities = trainingBalance.reduce((sum, b) => sum + b.total_opportunities, 0)

  // Prepare data in the correct order for the radar chart
  const radarData = CATEGORY_ORDER.map(categoryKey => {
    const category = trainingBalance.find(b => b.category === categoryKey)
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {d.value} opportunities ({d.percentage.toFixed(0)}%)
                  </div>
                </div>
              </div>
            )
